...
{
    try {
        // Determina se é criação ou edição baseado na presença do ID
        $isEdit = isset($_POST['id']) && !empty($_POST['id']);
        $action = $isEdit ? 'editar' : 'criar';
        
        // Campos obrigatórios (adicionado cargo)
        $required_fields = ['nome', 'idade', 'email', 'cpf', 'cep', 'cargo'];
        foreach ($required_fields as $field) {
            if (!isset($_POST[$field]) || empty(trim($_POST[$field]))) {
                error("Campo obrigatório '$field' não encontrado ou vazio");
            }
        }
